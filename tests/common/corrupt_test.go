...
func TestEtcdCorruptHash(t *testing.T) {
	...
	for _, tc := range clusterTestCases() {
		t.Run(tc.name, func(t *testing.T) {
			testCorruptTest(t, cc, ctx)
		})
	}
}

func testCorruptTest(t *testing.T, c interfaces.Client, ctx context.Context) {
	t.Helper()
	...
	// I don't know how to convert this part... here is a tentative:
...
// 1. Identify the target tests to be migrated
func TestPeriodicCheckDetectsCorruption(t *testing.T) {
	...
	corruptCheckTime := WithCorruptCheckTime(time.Second)
	options := config.ClusterConfig{CorruptCheckTime: time.Second}
	clus := testRunner.NewCluster(ctx, t,
		config.WithClusterConfig(options),
		config.WithDataDir(true),
		corruptCheckTime,
	)
